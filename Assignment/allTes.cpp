...
class Employee{
	...
	//SearchByID
		...
		void searchByName(Employee obj[], int n){
			...
			cin.ignore(); // clear newline from buffer
			getline(cin, textSearch); // use getline to allow spaces in names
			// isFound=0;
		...
		void searchByPosition(Employee obj[], int n){
			// int isFound;
			string textSearch;
			bool isFound = false; // Use bool for clarity
			cin.ignore(); // clear newline from buffer
			cout<<"Input Employee name to search : ";//cin>>textSearch;
			getline(cin, textSearch); // use getline to allow spaces in names
			// isFound=0;
			...
			for(int i=0;i<n;i++){
				if(textSearch == obj[i].getPosition()){
					if(!isFound){
						printHeader();
						isFound = true; // Set isFound to true if we find a match
					}					
					obj[i].output();
					break;
				}					
